...
export async function fetchWidgetsDataAsync() {
  const arrayLength = 75;
  const trimArrayLength = 50;
  let url = `https://www.figma.com/api/widgets/browse?sort_by=popular&sort_order=desc&resource_type=&page_size=${arrayLength}`;
  ...
  while (typeof url !== 'undefined') {
    ...
    //////////////////////////////////////
    totalAmount = {
      widgets: totalAmount.widgets + result.length,
      installs:
    ...
    const topCurrent = result.map((a) => {
      return {
        ...
        icon: `https://www.figma.com/community/widget/${a.id}/icon`,
  ...
  fse.outputJsonSync(`data/widgets/common.json`, topData);
  console.log(topData);
}

fetchWidgetsDataAsync();
